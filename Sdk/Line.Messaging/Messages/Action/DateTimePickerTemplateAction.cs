...
namespace Line.Messaging
{
    ...
    public class DateTimePickerTemplateAction : ITemplateAction
    {
        ...
        {
            var mode = (DateTimePickerMode)Enum.Parse(typeof(DateTimePickerMode), (string)dynamicObject?.mode, true);
            var format = GetDateTimeFormat(mode);

            DateTime? initial = null;
            if (dynamicObject?.initial != null)
                initial = DateTime.ParseExact((string)dynamicObject?.initial, format, null);

            DateTime? min = null;
            if (dynamicObject?.min != null)
                min = DateTime.ParseExact((string)dynamicObject?.min, format, null);

            DateTime? max = null;
            if (dynamicObject?.max != null)
                max = DateTime.ParseExact((string)dynamicObject?.max, format, null);
